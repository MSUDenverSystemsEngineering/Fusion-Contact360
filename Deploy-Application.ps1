...
Try {
	...
	[string]$appVendor = 'Fusion'
	[string]$appName = 'Contact360'
	[string]$appVersion = '1.0.333.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'fusioncontact360' -CheckDiskSpace -PersistPrompt

		## <Perform Pre-Installation tasks here>
		## Remove previous installations
		Copy-File -Path "$dirSupportFiles\ClickOnceUninstaller.exe" -Destination "$envWinDir\MSUDenver"
		Execute-Process -Path "$envWinDir\MSUDenver\ClickOnceUninstaller.exe" -Parameters "`"Fusion Contact360`"" -WindowStyle "Hidden"
		Execute-Process -Path "$envWinDir\MSUDenver\ClickOnceUninstaller.exe" -Parameters "`"Fusion Contact360 - 1 `"" -WindowStyle "Hidden"

		Remove-File -Path "$envCommonDesktop\Fusion Contact360*.appref-ms"
		Remove-Folder -Path "$envSystemDrive\CallCenterApp"
		Remove-Folder -Path "$envSystemDrive\testFusionAll"
		Remove-Folder -Path "$envSystemDrive\Fusion360Installer"
		Remove-Folder -Path "$envSystemDrive\Fusion"
		$exitCode = Remove-MSIApplications -Name "Fusion Agent Desktop" -AddParameters "ALLUSERS=1" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }

		## Remove shortcuts and per-user installations
		Remove-File -Path "$envCommonDesktop\CallCenterApp.lnk"

		Get-ChildItem -Path "$envSystemDrive\Users" -Force | Where-Object {$_.PSIsContainer -eq $true} | ForEach-Object {
			$FusionStartMenu = $_.FullName + "\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Fusion"
			$FusionDesktopClickOnce = $_.FullName + "\Desktop\Fusion Contact360.appref-ms"
			If ((Test-Path $FusionStartMenu) -eq $true) {
				Remove-Folder -Path $FusionStartMenu
			}
			If ((Test-Path $FusionDesktopClickOnce) -eq $true) {
				Remove-File -Path $FusionDesktopClickOnce
			}
		}

		## Remove registry entries
		[scriptblock]$HKCURegistrySettings = {
			Remove-RegistryKey -Key "HKCU\Software\Microsoft\Windows\CurrentVersion\RunOnce" -Name "Fusion360Install" -SID $UserProfile.SID
		}
		Invoke-HKCURegistrySettingsForAllUsers -RegistrySettings $HKCURegistrySettings

		## Uninstall ClickOnce application on next login
		Set-ActiveSetup -StubExePath "$envWinDir\MSUDenver\ClickOnceUninstaller.exe" -Arguments "`"Fusion Contact360`"" -Description "Fusion Contact360" -Key "Fusion Contact360 Uninstall" -Version "1"
		Set-ActiveSetup -StubExePath "$envWinDir\MSUDenver\ClickOnceUninstaller.exe" -Arguments "`"Fusion Contact360 - 1 `"" -Description "Fusion Contact360" -Key "Fusion Contact360 Uninstall 2" -Version "1"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'fusioncontact360' -CloseAppsCountdown 60
		...
		$exitCode = Execute-MSI -Action "Uninstall" -Path "FusionAgentDesktopSetup.msi" -AddParameters "ALLUSERS=1" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCMWXibKHKnjY9f
# vz5IttadSDCwBAenIUYWXWzcT0bnwKCCD4cwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# gAAwGQYJKoZIhvcNAQkDMQwGCisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIEbw
# qQ5TvBBad3OVplT8psKBGQ4EkZ+bcOpGlc5mztdXMA0GCSqGSIb3DQEBAQUABIIB
# AIz8xCxL6imly88wopNvu4r7nUaGNrnI5HhO7SbIgAQftTu2iD9h0HALke9iRRvy
# /dq0zo9+M3Vz13ZmHPuQaOr3KnqVy5ClbE1RaawA0VF2TLCwGVfm+Atzi4zWZT9n
# WZQdbCZUpEUnQmudHeGB+myjVCTngg2rWTlxhzLxbqYrgs60YM7jgeMFqkTNEVJa
# Sq+CDFDkQf/RnfMKVYmeIMT7izMNlFukvWvyNFVWpTfnQ9IzI4vX3aCvRCTGa4G9
# 4iZYjTxRuhc6STdnKK5/GILOECnSxPhy2xItrmSHwRKR8crOnIHuIHHLdfDKGgEL
# vuoqPoqOsRmGsuP84aXSJ86hggKiMIICngYJKoZIhvcNAQkGMYICjzCCAosCAQEw
...
# ATAcBgkqhkiG9w0BCQUxDxcNMTcwNjA1MjIzNjU2WjAjBgkqhkiG9w0BCQQxFgQU
# DC0secZddXI4IUFb5JZqtpZsK9QwgZ0GCyqGSIb3DQEJEAIMMYGNMIGKMIGHMIGE
...
# AQUABIIBACKImKxcaorketjvEWhFstYUl4lksAUaSx+1vjkPSxqPiuyYDz70kftx
# YxwzNkHCFU1kLvYV7uqhaON+ED4rt0b3fJyVlokVOruBT7VNncSg4yf8ZQY+BORo
# jACSEmZV988N1FYDyBRJz39TktV8jSij766Vk4eyibKPpgcp47ERm8rX8+B1c8Ae
# cAIbGm0p6KBcbl/2+7jKPwBgLv6QEo0T9hdPTFv1p8CQphETOqGLIMVAsUaan3qs
# zrh4EEHbycOSNGZQlgJG26IuUhu12zU0LppBVyeVg/nmMlzhY3tHdpy7hqE80BwF
# 3jC0TOd8Llx1wymr0X33R7IyvMm6bTw=
